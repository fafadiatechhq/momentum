"""
Report: Bench Report
Shows employees whose utilization is below the configured target — i.e. bench/idle.
"""
...
def execute(filters=None):
    ...
    columns = [
        ...
        {"fieldname": "idle_hours",         "label": "Idle Hours",         "fieldtype": "Float",                            "width": 110},
    ...
    query = """
        SELECT
            ...
            ROUND(
                SUM(tsd.hours)
                - SUM(CASE WHEN tsd.is_billable = 1 THEN tsd.hours ELSE 0 END), 2
            ) AS idle_hours,
            ROUND(
        ...
        HAVING utilization_percent < %(target)s
